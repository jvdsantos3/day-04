...
- **Phase / Task**: Phase 3 (Domain Services): T9 (`344a5f1`) done → T10 (BudgetService, `f7b1878`) done → T11 (contracts) pendente. Phase 4: T12 (`0661b22`) e T13 (`8572145`) done.
- **Completed**: Specify, Design, Tasks, T1–T10, T12, T13
- **In-progress**: none
- **Next step**: T11 (Pydantic contracts). Phase 4: T12 done → T13 done → T14 (indexer, usa `vector/client.get_or_create_collections` + `require_user_id` do T12 e `vector/embeddings.get_embeddings` do T13) → T15.

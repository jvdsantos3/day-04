...
- **Phase / Task**: Onda 9 (paralelo) COMPLETA — T9 (`344a5f1`), T12 (`0661b22`) e T13 (`8572145`) done. Phase 3 (Domain Services): T9 done → T10 (BudgetService), T11 (contracts) pendentes.
- **Completed**: Specify, Design, Tasks, T1–T9, T12, T13
- **In-progress**: none
- **Next step**: T10 (BudgetService — depende de T9) e/ou T11 (contracts). Phase 4: T12 done → T13 done → T14 (indexer, usa `vector/client.get_or_create_collections` + `require_user_id` do T12 e `vector/embeddings.get_embeddings` do T13) → T15.
- **Blockers**: none — o erro de coleção do T9 (`list[...]` sombreado pelo método `list`) foi resolvido com `from __future__ import annotations` no repositório; suíte unit inteira passa (58 testes).

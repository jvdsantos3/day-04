...
- **Phase / Task**: Onda 9 (paralelo) em andamento — T12 (chroma client) done; T9 (TransactionRepository) in-progress por outra sessão; T13 (embeddings) pendente
- **Completed**: Specify, Design, Tasks, T1–T8, T12
- **In-progress**: T9 (TransactionRepository) — arquivos untracked de outra sessão (ver Blockers)
- **Next step**: T13 (embeddings singleton, dim 384) e/ou finalizar T9. Phase 4 (Vector): T12 done → T13 → T14 (indexer, usa `vector/client.get_or_create_collections` + `require_user_id`) → T15.
- **Blockers**: `src/financial_assistant/domain/repositories/transaction_repository.py` (T9, untracked, de outra sessão) tem erro de coleção `TypeError: 'function' object is not subscriptable` na linha ~133 (provável `list[...]` sombreado por um nome `list`) — quebra a coleção da suíte unit inteira. NÃO commitado por esta sessão (fora do escopo T12). Autor do T9 deve corrigir antes do gate full da Phase 3.
- **Uncommitted files**: T9 untracked (`transaction_repository.py` + `test_transaction_repository.py`) de outra sessão — não tocados

...
- **Phase / Task**: Phase 2 (Auth) iniciada — T5 (password hashing) done; próxima T6 (register endpoint + template)
- **Completed**: Specify, Design, Tasks, T1, T2, T3, T4, T5
- **In-progress**: none
- **Next step**: Executar T6 — `POST /register` + `GET /register`, validação min 8 chars, email duplicado; usar `auth.service.hash_password`
- **Blockers**: none
